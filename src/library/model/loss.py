...
class FocalLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        neg_loss = (
            torch.log(one_minus_input) * torch.pow(input, 2) * neg_weights * neg_inds
        )  # (B, N, H, W)
        ...
        pos_loss = (
            torch.log(input) * torch.pow(one_minus_input, 2) * pos_inds
        )  # (B, N, H, W)
...
class LossStats:
    ...
    def __truediv__(self, value):
        return LossStats(
            self.hm_loss / value, self.offset_loss / value, self.embedding_loss / value
        )
