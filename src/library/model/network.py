...
class Network(nn.Module):
    def __init__(self, args, pretrained=True):
        ...
        net = YOLO(model="yolov8l-cls").model.model
        del net[-1]

        self.down1 = nn.Sequential(net[:3])  # /4 -> /4
        self.down2 = nn.Sequential(net[3:5])  # /2 -> /8
        self.down3 = nn.Sequential(net[5:7])  # /2 -> /16
        self.down4 = nn.Sequential(net[7:9])  # /2 -> /32

        self.up4 = nn.Conv2d(1024, self.fpn_depth, kernel_size=1)  # x1 -> /32
        self.up3 = Fpn(512, self.fpn_depth)  # x2 -> /16
        self.up2 = Fpn(256, self.fpn_depth)  # x2 -> /8
        self.up1 = Fpn(128, self.fpn_depth)  # x2 -> /4

        self.head = Head(self.fpn_depth, self.out_channels)

    def forward(self, x):  # (B, 3, H, W)
        p1 = self.down1(x)  # (B, 64, H/4, W/4)
        p2 = self.down2(p1)  # (B, 128, H/8, W/8)
        p3 = self.down3(p2)  # (B, 256, H/16, W/16)
        p4 = self.down4(p3)  # (B, 512, H/32, W/32)
